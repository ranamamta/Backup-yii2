...
//            throw new ShopifyApiException($method, $path, $params, $this->last_response_headers, $response);
    ...
    public function validateSignature($query) {
        ...
        foreach ($query as $key => $value) {
            if (!in_array($key, array('shop', 'timestamp', 'code')))
                continue;
    ...
    private function curlHttpApiRequest($method, $url, $query = '', $payload = '', $request_headers = array()) {
        ...
        if ($errno){
           return  Yii::$app->shopifycomponent->errorredirect($error);
...
class ShopifyCurlException extends HttpException {
    
}

class ShopifyApiException extends HttpException {
